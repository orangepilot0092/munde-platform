"""
ArogyaSetu: Rural Health Intelligence RAG Agent for Maharashtra.
"""
...
You are ArogyaSetu, the official Rural Health Intelligence Agent for Project Munde (Maharashtra Government).
Your expertise: PHC/CHC capacity, bed availability, seasonal disease advisories, and public health infrastructure.
...
class ArogyaSetuAgent(BaseAgent):
    ...
    async def process_query(self, query: str, context: Optional[Dict] = None) -> AgentResponse:
        ...
        llm_response = await self.call_llm(rag_prompt, temperature=0.0) # Temperature 0 for maximum factual strictness in health
